...
interface StudentNoticesViewProps {
  initialNotices: StudentNoticeItem[];
}

export function StudentNoticesView({ initialNotices }: StudentNoticesViewProps) {
  const { isOnline } = useNetworkStatus();
  const [notices, setNotices] = useState<StudentNoticeItem[]>(initialNotices || []);
  ...
  useEffect(() => {
    if (initialNotices && initialNotices.length > 0) {
      ...
      saveOfflineData('notices', initialNotices);
    } else {
      const cached = getOfflineData<StudentNoticeItem[]>('notices');
    ...
    <div className="space-y-6 md:space-y-8 max-w-4xl">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm sm:text-base mt-1.5">
          Stay updated with the latest news, schedules, and important alerts from Lamka Coaching.
        </p>
      </div>
      ...
      {notices.length > 0 ? (
        <div className="space-y-4 sm:space-y-6">
          {notices.map((notice) => (
            <Card
              key={notice.id}
              className={`border border-slate-200/80 shadow-xs bg-white rounded-2xl overflow-hidden transition-all hover:shadow-md ${
                notice.pinned ? 'border-blue-300 ring-1 ring-blue-100 bg-blue-50/20' : ''
              }`}
            >
              <CardHeader className="p-5 sm:p-6 pb-3">
                <div className="flex justify-between items-start gap-4">
                  ...
                  {notice.pinned && (
                    <Badge
                      variant="secondary"
                      className="bg-blue-100 text-blue-800 border-none shrink-0 text-xs font-semibold px-2.5 py-0.5 rounded-full"
                    >
                      Important
                    </Badge>
                  )}
        ...
        <Card className="border-2 border-dashed border-slate-200 shadow-none bg-white rounded-3xl p-8 sm:p-12">
          <CardContent className="flex flex-col items-center justify-center p-0 text-center">
            ...
            <p className="text-slate-500 text-sm mt-1.5 max-w-md">
              There are no new notices at the moment. We'll post important updates here.
            </p>
          </CardContent>
        </Card>
      )}
