...
import { saveOfflineData, getOfflineData } from '@/lib/offline/offline-store';
...
interface ScheduleViewProps {
  initialEnrollments: ScheduleEnrollment[];
}
...
export function ScheduleView({ initialEnrollments }: ScheduleViewProps) {
  const { isOnline } = useNetworkStatus();
  const [enrollments, setEnrollments] = useState<ScheduleEnrollment[]>(initialEnrollments || []);
  ...
  useEffect(() => {
    if (initialEnrollments && initialEnrollments.length > 0) {
      ...
      saveOfflineData('schedule', initialEnrollments);
    } else {
      // Try loading from offline cache
      const cached = getOfflineData<ScheduleEnrollment[]>('schedule');
      if (cached.isAvailable && cached.data) {
        setEnrollments(cached.data);
        setLastSyncTime(cached.formattedTime);
        setIsFromCache(true);
      }
    }
  }, [initialEnrollments]);

  // When network goes offline, verify cache timestamp
  useEffect(() => {
    if (!isOnline) {
      const cached = getOfflineData<ScheduleEnrollment[]>('schedule');
  ...
  return (
    <div className="space-y-6 md:space-y-8 max-w-5xl">
      {(!isOnline || isFromCache) && (
        <OfflineBanner lastSyncTime={lastSyncTime} />
      )}
      ...
      {hasEnrollments ? (
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-slate-100">
              {weekDays.map((day) => {
                ...
                const classes = activeEnrollments.map((enr) => {
                  ...
                  return {
                    ...
                    batchName: batch?.batchName || 'Default Batch',
                    department: course?.department?.name,
                    timing: batch?.timing || 'Timing Scheduled',
                    isStarted,
                    isEnded,
                    startDateFormatted: batchStart?.toLocaleDateString('en-IN', {
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    }),
                ...
                return (
                  ...
                  >
                    <div className="md:col-span-1 pt-1 flex md:flex-col items-center md:items-start justify-between md:justify-start gap-2">
                      ...
                      <span className="text-xs text-slate-500 font-medium">{day.dateFormatted}</span>
                    </div>
                    ...
                    <div className="md:col-span-3 space-y-2.5">
                      {classes.length > 0 ? (
                        ...
                        <div className="text-xs text-slate-600 italic py-2">No scheduled classes</div>
                      )}
                    </div>
                  </div>
                );
        ...
        <Card className="border-2 border-dashed border-slate-200 shadow-none bg-white rounded-3xl p-8 sm:p-12">
          <CardContent className="flex flex-col items-center justify-center p-0 text-center">
            ...
            <Button
              asChild
              className="rounded-xl h-10 px-6 font-semibold bg-cyan-600 hover:bg-cyan-700 text-white shadow-xs gap-2"
            >
